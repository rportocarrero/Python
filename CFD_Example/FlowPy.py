...
def SetUBoundary(space, left, right, top, bottom):
    ...
    if(top.type == "D"):
        space.u[-1,:] = 2 * top.value - space.v[-2,:]
    elif(top.type == "N"):
        space.u[-1,:] = - top.value * space.dy + space.v[-2,:]

    if(bottom.type == "D"):
        space.u[0,:] = 2 * bottom.value-space.u[1,:]
    elif(bottom.type == "N"):
        space.u[0,:] = bottom.value * space.dy + space.v[1,:]
...
def SetTimeStep(CFL, space, fluid):
    with np.errstate(divide = 'ignore'):
        dt = CFL / np.sum([np.amax(space.v) / space.dx, np.amax(space.v) / space.dy])
...
def GetStarredVelocities(space, fluid):
    ...
    v2_x = (v[1:rows+1, 1:cols+1] + u[1:rows+1, 2:] + u[0:rows, 1:cols+1] + u[0:rows, 2:]) /4
...
def SolvePressurePoisson(space, fluid, left, right, top, bottom):
    #Save object attributes as local variable with typing for readability
    rows = int(space.rowpts)
    cols = int(space.colpts)
    ...
    #Evalute derivative of starred velocities
    ...
    while(error>tol):
        ...
        # Find manximum error between old nad new pressure matrices
        error = np.amax(abs(p-p_old))

        # Apply pressure boundary conditions
        SetPBoundary(space, left, right, top, bottom)

        # Escape condition for nonconvergence
        if(i>500):
            tol *= 10

def SolveMomentumEquation(space, fluid):
    # save object attributes as local variable with typing for readbility
    rows = int(space.rowpts)
    cols = int(space.colpts)
    u_star = space.u_star.astype(float,copy=False)
    v_star = space.v_star.astype(float, copy=False)
